...
export default function Cart() {
  ...
  const calculateTotalAmount = () => {
    const totalAmount = cart.reduce((total, product) => total + product.price * product.quantityInCart, 0);
          ...
          <TableBody>
            {cart.map((product) => (
              <TableRow key={product.id}>
                  ...
                  <div className='flex items-center gap-3'>
                    <div className='rounded border bg-white p-1'>
                      <div className='relative h-12 w-12'>
                        <Image src={product.image} alt='product image' sizes='100%' fill className='object-contain' />
                      </div>
                    </div>
                ...
                <TableCell className='text-right'>${calculateSubTotal(product).toFixed(2)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
          <TableFooter>
