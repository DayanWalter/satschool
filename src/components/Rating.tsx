...
const Rating = ({ rating }: { rating: number }) => {
  const renderStars = () => {
    ...
    for (let i = 0; i < fullStars; i++) {
      stars.push(<Icon key={i} className='text-primary' path={mdiStar} size={1} />);
    }

    // If the rating has a half star, push this star icon into the stars array
    if (hasHalfStar) {
      stars.push(<Icon key='half' className='text-primary' path={mdiStarHalfFull} size={1} />);
    }
    ...
    for (let i = 0; i < remainingStars; i++) {
      stars.push(
        <Icon key={i + fullStars + (hasHalfStar ? 1 : 0)} className='text-primary' path={mdiStarOutline} size={1} />,
      );
    }
